...
int main(int argc, char *argv[])
{
    ...
    uint8_t buf1[10] = {0x67, 0x64, 0x00, 0x1E, 0xAC, 0xD9, 0x40, 0xD4, 0x3D,0xA1}, buf2[6] = {0x68, 0xEB, 0xE3, 0xCB,0x22,0xC0}, buf3[2] = {0x15, 0x88};
    //创建 mp4 文件
    if (argc < 3){
    ...
    // 添加 h264 track
    video = MP4AddH264VideoTrack(file, VIDEO_TIME_SCALE, VIDEO_TIME_SCALE / 25,640, 480,0x64, // sps[1]
                                 0x00, // sps[2]
                                 0x1f, // sps[3]
                                 3);
    ...
    MP4AddH264SequenceParameterSet(file, video, buf1, 10);
    MP4AddH264PictureParameterSet(file, video, buf2, 6);

    // 添加 AAC track
    audio = MP4AddAudioTrack(file, AUDIO_TIME_SCALE, AUDIO_TIME_SCALE / 8,MP4_MPEG4_AUDIO_TYPE);
