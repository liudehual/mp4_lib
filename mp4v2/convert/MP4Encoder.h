...
///#pragma comment(lib, "F:/mp4v2/lib/libmp4v2.lib")
...
public:
  ...
  MP4FileHandle CreateMP4File(const char *fileName,int width,int height,int timeScale = 90000,int frameRate = 30);
  // wirte 264 metadata in mp4 file.
  bool Write264Metadata(MP4FileHandle hMp4File,LPMP4ENC_Metadata lpMetadata);
  bool write264Sps(MP4FileHandle hMp4File,char *buf,int size);
  bool write264Pps(MP4FileHandle hMp4File,char *buf,int size);
