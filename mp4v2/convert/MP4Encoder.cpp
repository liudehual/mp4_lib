...
bool MP4Encoder::write264Sps(MP4FileHandle hMp4File,char *buf,int size)
{

  return false;
}
bool MP4Encoder::write264Pps(MP4FileHandle hMp4File,char *buf,int size)
{
...
int MP4Encoder::WriteH264Data(MP4FileHandle hMp4File,const unsigned char* pData,int size)
{
  ...
  while (len = ReadOneNaluFromBuf(pData,size,pos,nalu)){
      fprintf(stderr,"nalu type %02X nalu size %d \n",nalu.type,nalu.size);

      if(nalu.type == 0x07 && !m_sps_init){ /// sps
          ...
          m_sps_init=1;
        }else if(nalu.type == 0x08 && !m_pps_init){ /// pps
          /// 只初始化一次，多次会出问题
      ...
      /// 必须初始化sps,pps
      if( !m_sps_init || !m_pps_init || nalu.type==0x06){
          pos += len;
          continue;
        }
      ...
      if(nalu.type==0x05){
          if(!MP4WriteSample(hMp4File, m_videoId, data, datalen,MP4_INVALID_DURATION)){
              return 0;
            }
        }else{
          if(!MP4WriteSample(hMp4File, m_videoId, data, datalen,MP4_INVALID_DURATION,0,0)){
              return 0;
            }
        }
...
bool MP4Encoder::WriteH264File(const char* pFile264,const char* pFileMp4)
{
  ...
  hMp4file = CreateMP4File(pFileMp4,848,480);
  ...
  while(1){
      ...
      writelen = WriteH264Data(hMp4file,buffer,readlen);
